...
// -----------------------------------------------------------------------------
const char* ELRS_BIND_PHRASE = "CHANGE_ME";  // Same wording as in ExpressLRS Configurator
// Replace with the MAC address of the ESP32 receiver (print WiFi.macAddress() on the RX)
...
const int MODE_SELECT_PIN = -1;      // Set to a GPIO if you want a physical switch (LOW = wired, HIGH = wireless)
const bool MODE_SELECT_PULLUP = true;

// UART (ELRS bridge) settings
#include <HardwareSerial.h>
HardwareSerial elrsSerial(1);
const int ELRS_UART_TX_PIN = 17;
const int ELRS_UART_RX_PIN = -1;      // Not used; set to a GPIO if you want bidirectional data
const unsigned long ELRS_UART_BAUD = 115200;
bool elrsReady = false;

// ESP-NOW settings
bool espNowReady = false;
uint8_t espNowPeer[6] = {0};   // Populated from ESP_NOW_PEER_MAC in setup()
...
void setup() {
  ...
  if (MODE_SELECT_PIN >= 0) {
    ...
    outputMode = high ? MODE_UART_ELRS : MODE_WIRED_PWM;
  }
  ...
  Serial.println("Commands: 'center', 'debug', 'limits', 'test', 'deadband X', 'smooth Y', 'mode pwm|uart|espnow'");
...
void processSerialCommand(String command) {
  ...
  if (command == "center") {
    ...
    if (command.endsWith("pwm")) {
      ...
      setOutputMode(MODE_UART_ELRS, true);
    } else if (command.endsWith("espnow")) {
      setOutputMode(MODE_ESPNOW, true);
    } else {
      Serial.println("Unknown mode. Use 'mode pwm', 'mode uart', or 'mode espnow'.");
    }
    ...
    Serial.println("Unknown command. Available: center, debug, limits, test, deadband X, smooth Y, mode pwm|uart|espnow");
  }
}
...
void transmitWirelessTargets(bool ppmValid) {
  ...
  if (outputMode == MODE_UART_ELRS) {
    if (!elrsReady) initElrsUart();
    if (elrsReady) {
      elrsSerial.write(reinterpret_cast<uint8_t*>(&pkt), sizeof(pkt));
    }
  } else if (outputMode == MODE_ESPNOW) {
...
void initElrsUart() {
  ...
  elrsReady = true;
  Serial.println("ELRS UART bridge ready.");
}
...
void initEspNow() {
  ...
  espNowReady = true;
  Serial.println("ESP-NOW link ready.");
}
